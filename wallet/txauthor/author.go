...
func NewUnsignedTransaction(outputs []*wire.TxOut, feeRatePerKb chainutil.Amount,
...
func CalculateTransactionFee(outputs []*wire.TxOut, feeRatePerKb chainutil.Amount,
	fetchInputs InputSource, changeSource *ChangeSource) (*chainutil.Amount, error) {
	...
	for {
		inputAmount, _, _, scripts, err := fetchInputs(targetAmount + targetFee)
		if err != nil {
			return nil, err
		}
		if inputAmount < targetAmount+targetFee {
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte,
	inputValues []chainutil.Amount, secrets SecretsSource) error {
...
func TXPrevOutFetcher(tx *wire.MsgTx, prevPkScripts [][]byte,
	inputValues []chainutil.Amount) (*txscript.MultiPrevOutFetcher, error) {
