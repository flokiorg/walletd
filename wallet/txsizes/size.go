...
// EstimateInputSize returns the worst-case serialized size estimate for a transaction input. // #FLZ_CHANGE
//   - 32 bytes: Previous transaction hash
//   - 4 bytes: Output index
//   - Compact size (VarInt): Script size (depends on actual script length)
//   - N bytes: Signature script (provided scriptSize parameter)
//   - 4 bytes: Sequence number
func EstimateInputSize(scriptSize int) int {
	return 32 + 4 + wire.VarIntSerializeSize(uint64(scriptSize)) + scriptSize + 4
}

// EstimateOutputSize returns the worst-case serialized size estimate for a transaction output.
//   - 8 bytes: Amount (satoshis)
//   - Compact size (VarInt): Script size (depends on actual script length)
//   - N bytes: Locking script (provided scriptSize parameter)
func EstimateOutputSize(scriptSize int) int {
	return 8 + wire.VarIntSerializeSize(uint64(scriptSize)) + scriptSize
}
