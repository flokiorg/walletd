...
var (
	testCases = []*testCase{{
		...
		expectedAddr:       "tf1qllxcutkzsukf8u8c8stkp464j0esu9xq0jpjsw",
		expectedChangeAddr: "tf1qu6jmqglrthscptjqj3egx54wy8xqvzn5xzvxek",
	}, {
		...
		expectedScope:      waddrmgr.KeyScopeBIP0049Plus,
		expectedAddr:       "2NBCJ9WzGXZqpLpXGq3Hacybj3c4eHRcqgh",
		expectedChangeAddr: "tf1qeqn05w2hfq6axpdprhs4y7x65gxkkvfv45ncz5",
	}, {
		name: "bip84",
		...
		addrType:           waddrmgr.WitnessPubKey,
		expectedScope:      waddrmgr.KeyScopeBIP0084,
		expectedAddr:       "tf1q5vepvcl0z8xj7kps4rsux722r4dvfwlh8gp6xz",
		expectedChangeAddr: "tf1qlwe2kgxcsa8x4huu79yff4rze0l5mwafexts39",
	}}
)
...
func testImportAccount(t *testing.T, w *Wallet, tc *testCase, watchOnly bool,
	...
	case waddrmgr.NestedWitnessPubKey:
		witnessAddr, err := chainutil.NewAddressWitnessPubKeyHash(
			chainutil.Hash160(acct3ExternalPub.SerializeCompressed()),
		...
		intAddr, err = chainutil.NewAddressScriptHash(
			witnessProg, &chaincfg.TestNet3Params,
		)
		require.NoError(t, err)

	case waddrmgr.WitnessPubKey:
		intAddr, err = chainutil.NewAddressWitnessPubKeyHash(
			chainutil.Hash160(acct3ExternalPub.SerializeCompressed()),
			&chaincfg.TestNet3Params,
		)
		require.NoError(t, err)
