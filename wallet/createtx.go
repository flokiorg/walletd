...
func makeInputSource(eligible []Coin) txauthor.InputSource {
	...
	return func(target chainutil.Amount) (chainutil.Amount, []*wire.TxIn,
		...
		for currentTotal < target && len(eligible) != 0 {
			...
			currentTotal += chainutil.Amount(prevOut.Value)
			...
			currentInputValues = append(
				currentInputValues,
				chainutil.Amount(prevOut.Value),
			)
		}
...
func (s secretSource) GetKey(addr chainutil.Address) (*crypto.PrivateKey, bool, error) {
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut,
	...
	account uint32, minconf int32, feeSatPerKb chainutil.Amount,
...
func (w *Wallet) txFee(outputs []*wire.TxOut,
	coinSelectKeyScope, changeKeyScope *waddrmgr.KeyScope,
	account uint32, minconf int32, feeSatPerKb chainutil.Amount,
	...
	*chainutil.Amount, error) {
	...
	var feeAmount *chainutil.Amount
	err = walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		feeAmount, err = txauthor.CalculateTransactionFee(
			outputs, feeSatPerKb, inputSource, changeSource,
		)
		if err != nil {
			return err
...
// transaction.
func inputYieldsPositively(credit *wire.TxOut, feeRatePerKb chainutil.Amount) bool {

	inputSize := txsizes.GetMinInputVirtualSize(credit.PkScript)
	inputFee := feeRatePerKb * chainutil.Amount(inputSize) / 1000

	return inputFee < chainutil.Amount(credit.Value)
}
...
func (w *Wallet) addrMgrWithChangeSource(dbtx walletdb.ReadWriteTx,
	...
	newChangeScript := func() ([]byte, error) {
		...
		// account 0.
		var (
			changeAddr chainutil.Address
			err        error
		)
		if account == waddrmgr.ImportedAddrAccount {
			changeAddr, err = w.newChangeAddress(
				addrmgrNs, 0, *changeKeyScope, 1,
			)
		} else {
			changeAddr, err = w.newChangeAddress(
				addrmgrNs, account, *changeKeyScope, 1,
			)
		}
		if err != nil {
			return nil, err
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte,
	inputValues []chainutil.Amount) error {
...
func (*RandomCoinSelector) ArrangeCoins(eligible []Coin,
	feeSatPerKb chainutil.Amount) ([]Coin, error) {
