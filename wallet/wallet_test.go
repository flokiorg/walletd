...
var (
	...
	TstTx, _           = chainutil.NewTxFromBytes(TstSerializedTx)
...
func TestDuplicateAddressDerivation(t *testing.T) {
	...
	var (
		m           sync.Mutex
		globalAddrs = make(map[string]chainutil.Address)
	)
	...
	for o := 0; o < 10; o++ {
		...
		for n := 0; n < 10; n++ {
			eg.Go(func() error {
				addrs := make([]chainutil.Address, 10)
				for i := 0; i < 10; i++ {
...
func TestEndRecovery(t *testing.T) {
	...
	// when using walletd with a fresh seed, because it requires an early
