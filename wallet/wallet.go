...
import (
	...
	"github.com/flokiorg/walletd/walletdb/migration"
...
type CoinSelectionStrategy interface {
	...
	ArrangeCoins(eligible []Coin, feeSatPerKb chainutil.Amount) ([]Coin,
		error)
}
...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadWriteTx) ([]chainutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	var addrs []chainutil.Address
	err := w.Manager.ForEachRelevantActiveAddress(
		addrmgrNs, func(addr chainutil.Address) error {
...
func (w *Wallet) syncWithChain(birthdayStamp *waddrmgr.BlockStamp) error {
	...
	// so now.
	if w.recoveryWindow > 0 { // #FLZ_CHANGE
...
func (w *Wallet) CreateSimpleTx(coinSelectKeyScope *waddrmgr.KeyScope,
	...
	satPerKb chainutil.Amount, coinSelectionStrategy CoinSelectionStrategy,
...
func (w *Wallet) SimpleTxFee(coinSelectKeyScope *waddrmgr.KeyScope,
	account uint32, outputs []*wire.TxOut, minconf int32,
	satPerKb chainutil.Amount, coinSelectionStrategy CoinSelectionStrategy,
	optFuncs ...TxCreateOption) (*chainutil.Amount, error) {
	...
	return w.txFee(
		outputs, coinSelectKeyScope,
		opts.changeKeyScope, account, minconf,
		satPerKb, coinSelectionStrategy,
		opts.selectUtxos, opts.allowUtxo,
	)
}
...
// CurrentAddress gets the most recently requested Flokicoin payment address
...
// blockchain or flokicoind mempool), the next chained address is returned.
...
func (w *Wallet) PubKeyForAddress(a chainutil.Address) (*crypto.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a chainutil.Address) (*crypto.PrivateKey, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		// is calculated.
		feeF64 = (outputTotal - debitTotal).ToFLC()
	}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := chainutil.Amount(output.Value).ToFLC()
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					_, ok = pkHashes[apkh.EncodeAddress()]
					if !ok {
						continue
					}
...
func (w *Wallet) ListUnspent(minconf, maxconf int32,
	accountName string) ([]*chainjson.ListUnspentResult, error) {
...
// NewAddressRPCLess returns the next external chained address for a wallet without any notification.
func (w *Wallet) NewAddressRPCLess(account uint32,
	scope waddrmgr.KeyScope, numAddresses uint32) (chainutil.Address, error) {
	...
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
		addr, _, err = w.newAddress(addrmgrNs, account, scope, numAddresses)
		return err
	})
	if err != nil {
		return nil, err
...
func (w *Wallet) NewAddress(account uint32,
	...
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
		addr, props, err = w.newAddress(addrmgrNs, account, scope, 1)
		return err
	})
	if err != nil {
		return nil, err
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope, numAddresses uint32) (chainutil.Address, *waddrmgr.AccountProperties, error) {
	...
	addrs, err := manager.NextExternalAddresses(addrmgrNs, account, numAddresses)
	if err != nil {
		return nil, nil, err
	}
...
func (w *Wallet) NewChangeAddressRPCLess(account uint32,
	scope waddrmgr.KeyScope, numAddresses uint32) (chainutil.Address, error) {
	...
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
		addr, err = w.newChangeAddress(addrmgrNs, account, scope, numAddresses)
		return err
	})
	if err != nil {
		return nil, err
...
func (w *Wallet) NewChangeAddress(account uint32,
	...
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
		addr, err = w.newChangeAddress(addrmgrNs, account, scope, 1)
		return err
	})
	if err != nil {
		return nil, err
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, scope waddrmgr.KeyScope, numAddresses uint32) (chainutil.Address, error) {
	...
	addrs, err := manager.NextInternalAddresses(addrmgrNs, account, numAddresses)
	if err != nil {
		return nil, err
	}
...
// returning the total amount of Flokicoin received for all accounts.
...
// returning the total amount of flokicoins received for a single wallet
// address.
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, keyScope *waddrmgr.KeyScope,
	account uint32, minconf int32, satPerKb chainutil.Amount,
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr chainutil.Address) (*crypto.PrivateKey, bool, error) {
...
func (w *Wallet) reliablyPublishTransaction(tx *wire.MsgTx,
	...
	// the transaction.
	var ourAddrs []chainutil.Address
	err = walletdb.Update(w.db, func(dbTx walletdb.ReadWriteTx) error {
...
// in order to allow applications wrapping walletd to store app-specific data
...
func (w *Wallet) DeriveFromKeyPath(scope waddrmgr.KeyScope,
	path waddrmgr.DerivationPath) (*crypto.PrivateKey, error) {
