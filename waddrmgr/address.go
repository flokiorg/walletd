...
import (
	...
	"github.com/flokiorg/walletd/internal/zero"
...
type ValidatableManagedAddress interface {
	...
	Validate(msg [32]byte, priv *crypto.PrivateKey) error
}

// ManagedScriptAddress extends ManagedAddress and represents a pay-to-script-hash
// style of flokicoin addresses.  It additionally provides information about the
// script.
...
func (a *managedAddress) PubKey() *crypto.PublicKey {
	return a.pubKey
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) PrivKey() (*crypto.PrivateKey, error) {
	...
	privKey, _ := crypto.PrivKeyFromBytes(privKeyCopy)
...
func (a *managedAddress) Validate(msg [32]byte, priv *crypto.PrivateKey) error {
	...
	// the public key to a Flokicoin address.
	...
	addrPrivKey, _ := crypto.PrivKeyFromBytes(a.privKeyCT)
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	derivationPath DerivationPath, pubKey *crypto.PublicKey, compressed bool,
	...
	case NestedWitnessPubKey:
		...
		// backwards compatible to Flokicoin nodes running 0.6.0 onwards, but
...
func newManagedAddress(s *ScopedKeyManager, derivationPath DerivationPath,
	privKey *crypto.PrivateKey, compressed bool,
...
//
// This is part of the ManagedAddress interface implementation.
func (a *witnessScriptAddress) Address() chainutil.Address {
	return a.address
}
...
func newWitnessScriptAddress(m *ScopedKeyManager, account uint32, scriptIdent,
	...
	case witnessVersionV0:
		address, err := chainutil.NewAddressWitnessScriptHash(
...
type taprootScriptAddress struct {
	...
	TweakedPubKey *crypto.PublicKey
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *taprootScriptAddress) Address() chainutil.Address {
	return a.address
}
