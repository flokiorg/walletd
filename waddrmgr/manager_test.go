...
func testExternalAddresses(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i := 0; i < len(expectedExternalAddrs); i++ {
			...
			utilAddr, err := chainutil.NewAddressPubKeyHash(
				pkHash, chainParams,
			)
			if err != nil {
...
func testImportPrivateKey(tc *testContext) bool {
	tests := []struct {
	...
	}{
		{
			...
			expected: expectedAddr{
				address:     "FLzmAVYYicNxXe3gBFGgdTjKyZHv9iTq25",
		...
		},
		{
			...
			expected: expectedAddr{
				address:     "FRdbj27WiTv8HdFLEbBURDWUeFewtzuYsN",
...
func testImportScript(tc *testContext) bool {
	tests := []struct {
		...
		},
		{
			...
			expected: expectedAddr{
				address:     "fc1q0jljr70qchwtk3ag0w3gyg9mjhg4c95xr7h8ezhvdrfgppcpz4esdyzphf",
		...
		},
		{
			...
			expected: expectedAddr{
				address:         "fc1q3a79gkjulrsgp864yskp4d5zmwm49xsdrfwvdypkqtlpj7spd3fq8kfhn2",
		...
		},
		{
			...
			expected: expectedAddr{
				address:     "fc1pu92qt24cl4spyp4rsj9sa3y4ma6a3fszgewcmway9f6f80vgnduqskwf9x",
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			// new addresses and ensure they're accurate.
			var (
				utilAddr chainutil.Address
				err      error
			)
			switch {
			case test.isWitness && test.witnessVersion == 0:
				scriptHash := sha256.Sum256(test.in)
				utilAddr, err = chainutil.NewAddressWitnessScriptHash(
			...
			case test.isTaproot:
				...
				utilAddr, err = chainutil.NewAddressTaproot(
					schnorr.SerializePubKey(taprootKey),
					chainParams,
				)

			default:
				utilAddr, err = chainutil.NewAddressScriptHash(
					test.in, chainParams,
				)
			}
			if err != nil {
...
func testConvertWatchingOnly(tc *testContext) bool {
	...
	db, err := walletdb.Open("bdb", woMgrName, true, defaultDBTimeout, false)
	if err != nil {
...
func deriveTestAccountKey(t *testing.T) *hdkeychain.ExtendedKey {
	scopeKey, err := deriveCoinTypeKey(rootKey, KeyScopeBIP0044)
	if err != nil {
...
func TestDeriveFromKeyPathCache(t *testing.T) {
	...
	var derivedKey *crypto.PrivateKey
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		ns := tx.ReadWriteBucket(waddrmgrNamespaceKey)
...
func TestManagedAddressValidation(t *testing.T) {
	...
	var testCases = []struct {
		name        string
		mutator     func(addr *managedAddress, ogPrivKey *crypto.PrivateKey)
		expectedErr error
	}{
		{
			...
			mutator: func(addr *managedAddress, priv *crypto.PrivateKey) {
		...
		},
		{
			...
			name: "addr bitflip",
			mutator: func(addr *managedAddress, priv *crypto.PrivateKey) {
				hash160 := chainutil.Hash160(
					addr.pubKey.SerializeCompressed(),
				)
				hash160[0] ^= 0x01

				addr.address, err = chainutil.NewAddressWitnessPubKeyHash(
