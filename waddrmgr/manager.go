...
// would end up being the same flokicoin address (as is the case for
...
type AccountProperties struct {
	...
	// AccountPrivKey is the account's private key.
	// Wallet must be unlocked to get access to this field.
	//
	// NOTE: This may be nil for imported accounts.
	AccountPrivKey *hdkeychain.ExtendedKey
...
func deriveCoinTypeKey(masterNode *hdkeychain.ExtendedKey,
	...
	purpose, err := masterNode.Derive( // nolint:staticcheck
	...
	coinTypeKey, err := purpose.Derive( // nolint:staticcheck
...
func deriveAccountKey(coinTypeKey *hdkeychain.ExtendedKey,
	...
	return coinTypeKey.Derive( // nolint:staticcheck
...
func checkBranchKeys(acctKey *hdkeychain.ExtendedKey) error {
	...
	if _, err := acctKey.Derive(ExternalBranch); err != nil { // nolint:staticcheck
		return err
	}

	// Derive the internal branch as the second child of the account key.
	_, err := acctKey.Derive(InternalBranch) // nolint:staticcheck
	return err
}
