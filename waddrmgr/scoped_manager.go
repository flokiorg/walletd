...
import (
	...
	"github.com/flokiorg/walletd/netparams"
...
func ScriptHashIdentity(script []byte) Identity {
	return func() []byte {
		return chainutil.Hash160(script)
	}
}
...
func TaprootIdentity(taprootKey *crypto.PublicKey) Identity {
	return func() []byte {
...
func (s *ScopedKeyManager) deriveKey(acctInfo *accountInfo, branch,
	...
	branchKey, err := acctKey.Derive(branch) // nolint:staticcheck
	if err != nil {
	...
	addressKey, err := branchKey.Derive(index) // nolint:staticcheck
...
func (s *ScopedKeyManager) AccountProperties(ns walletdb.ReadBucket,
	...
	if account != ImportedAddrAccount {
		...
		props.AccountPrivKey = acctInfo.acctKeyPriv
...
func (s *ScopedKeyManager) DeriveFromKeyPathCache(
	kp DerivationPath) (*crypto.PrivateKey, error) {
...
func (s *ScopedKeyManager) importedAddressRowToManaged(row *dbImportedAddressRow) (ManagedAddress, error) {
	...
	compressed := len(pubBytes) == crypto.PubKeyBytesLenCompressed
...
func (s *ScopedKeyManager) nextAddresses(ns walletdb.ReadWriteBucket,
	...
	branchKey, err := acctKey.Derive(branchNum) // nolint:staticcheck
	if err != nil {
		str := fmt.Sprintf("failed to derive extended key branch %d",
			branchNum)
		return nil, managerError(ErrKeyChain, str, err)
	...
	for i := uint32(0); i < numAddresses; i++ {
		...
		for {
			...
			key, err := branchKey.Derive(nextIndex) // nolint:staticcheck
			if err != nil {
...
func (s *ScopedKeyManager) ImportPublicKey(ns walletdb.ReadWriteBucket,
	pubKey *crypto.PublicKey, bs *BlockStamp) (ManagedAddress, error) {
...
func (s *ScopedKeyManager) importPublicKey(ns walletdb.ReadWriteBucket,
	...
	case PubKeyHash, WitnessPubKey:
		addressID = chainutil.Hash160(serializedPubKey)
	...
	case NestedWitnessPubKey:
		...
		addressID = chainutil.Hash160(witnessScript)

	case TaprootPubKey:
		internalPubKey, err := crypto.ParsePubKey(serializedPubKey)
		if err != nil {
			return err
		}
...
func (s *ScopedKeyManager) toImportedPrivateManagedAddress(
	wif *chainutil.WIF) (*managedAddress, error) {
...
func (s *ScopedKeyManager) toImportedPublicManagedAddress(
	pubKey *crypto.PublicKey, compressed bool) (*managedAddress, error) {
