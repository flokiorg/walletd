...
type FlokicoindConfig struct {
	...
	// Dialer is a closure we'll use to dial Flokicoin peers. If the chain
...
// Dialer represents a way to dial Flokicoin peers. If the chain backend is
...
func isBlockPrunedErr(err error) bool {
	...
	return ok && rpcErr.Code == chainjson.ErrRPCMisc &&
