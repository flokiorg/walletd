...
func newNetworkBlockTestHarness(t *testing.T, numBlocks,
	...
	h.dispatcher, err = NewPrunedBlockDispatcher(&PrunedBlockDispatcherConfig{
		...
		GetPeers: func() ([]chainjson.GetPeerInfoResult, error) {
			...
			res := make([]chainjson.GetPeerInfoResult, 0, len(h.peers))
			for addr, peer := range h.peers {
				...
				res = append(res, chainjson.GetPeerInfoResult{
					Addr:     addr,
		...
		GetNodeAddresses: func(*int32) ([]chainjson.GetNodeAddressesResult, error) {
			...
			res := make(
				[]chainjson.GetNodeAddressesResult, 0,
				len(h.fallbackAddrs),
			)
			for addr, peer := range h.fallbackAddrs {
				res = append(res, chainjson.GetNodeAddressesResult{
...
func TestPrunedBlockDispatcherMultipleQueryPeers(t *testing.T) {
	...
	for i := 0; i < numBlocks; i++ {
		h.assertPeerQueried()
		h.assertPeerReplied(blockChans[i], errChans[i], cancelChans[i], true)
	}
}
