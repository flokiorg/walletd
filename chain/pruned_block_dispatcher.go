...
// queryPeer represents a Flokicoin network peer that we'll query for blocks.
...
// SubscribeRecvMsg adds a OnRead subscription to the peer. All flokicoin messages
...
type PrunedBlockDispatcherConfig struct {
	...
	// Dial establishes connections to Flokicoin peers. This must support
	...
	GetPeers func() ([]chainjson.GetPeerInfoResult, error)
	...
	GetNodeAddresses func(*int32) ([]chainjson.GetNodeAddressesResult, error)
...
// "full-node".
func filterPeers(peers []chainjson.GetPeerInfoResult) ([]string, error) {
...
// "full-node".
func filterNodeAddrs(nodeAddrs []chainjson.GetNodeAddressesResult) []string {
	var eligible []string // nolint:prealloc
...
func (d *PrunedBlockDispatcher) handleResp(req, resp wire.Message,
	...
	err := blockchain.CheckBlockSanity(
		chainutil.NewBlock(block), d.cfg.ChainParams.PowLimit,
		d.timeSource,
	)
	if err != nil {
		d.blockMtx.Unlock()
	...
	err = blockchain.ValidateWitnessCommitment(chainutil.NewBlock(block))
	if err != nil {
		d.blockMtx.Unlock()
