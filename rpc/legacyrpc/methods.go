...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by walletd due to
...
// unsupported handles a standard flokicoind RPC request which is
// unsupported by walletd due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &chainjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by walletd",
	}
}
...
// information about the current state of walletd.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to flokicoind for all of the information in this command known
	// by them.
...
// appear in the blockchain, or any tx that has arrived in the flokicoind mempool).
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return (bals.Total - bals.Spendable).ToFLC(), nil
}
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return results[acctIndex].TotalReceived.ToFLC(), nil
}
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	if len(details.Debits) == 0 {
		...
		ret.Details[0] = chainjson.GetTransactionDetailsResult{
			...
			Category: "send",
			Amount:   (-debitTotal).ToFLC(), // negative since it is a send
			Fee:      &feeF64,
		}
		ret.Fee = feeF64
	}
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, chainjson.GetTransactionDetailsResult{
			...
			Address:  address,
			Category: credCat,
			Amount:   cred.Amount.ToFLC(),
			Vout:     cred.Index,
		})
	}

	ret.Amount = creditTotal.ToFLC()
	return ret, nil
}
...
func help(icmd interface{}, _ *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// flokicoind returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to flokicoind.  Therefore, create a
	...
	// texts in a non-internal flokicoind package.
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		accountBalances[result.AccountName] = result.AccountBalance.ToFLC()
	}
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		jsonResults = append(jsonResults, chainjson.ListReceivedByAccountResult{
			...
			Amount:        result.TotalReceived.ToFLC(),
...
func listReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	ret := make([]chainjson.ListReceivedByAddressResult, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = chainjson.ListReceivedByAddressResult{
			Address:       address,
			Amount:        addrData.amount.ToFLC(),
...
func sendPairs(w *wallet.Wallet, amounts map[string]chainutil.Amount,
	...
	feeSatPerKb chainutil.Amount) (string, error) {
	...
	if err != nil {
		...
		if _, ok := err.(chainjson.RPCError); ok {
			return "", err
		}
...
func sendToAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	// sendtoaddress always spends from the default account, this matches flokicoind
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	_ = wire.WriteVarString(&buf, 0, "Flokicoin Signed Message:\n")
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// TODO: really we probably should look these up with flokicoind anyway to
	...
	// querying flokicoind with getrawtransaction. We queue up a bunch of async
