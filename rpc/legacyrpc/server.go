...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="walletd RPC"`)
...
// functional flokicoin wallet RPC server.  This can be called to enable RPC
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := chainjson.MarshalResponse(
						chainjson.RpcVersion1, req.ID,
						resp, jsonErr,
					)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := chainjson.MarshalResponse(
			chainjson.RpcVersion1, req.ID, nil,
			chainjson.ErrRPCInvalidRequest,
		)
		if err != nil {
	...
	// Marshal and send.
	mresp, err := chainjson.MarshalResponse(
		chainjson.RpcVersion1, req.ID, res, jsonErr,
	)
	if err != nil {
		log.Errorf("Unable to marshal response: %v", err)
