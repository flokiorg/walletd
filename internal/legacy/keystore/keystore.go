...
import (
	...
	"github.com/flokiorg/walletd/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := crypto.PrivKeyFromBytes(privkey)
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	...
	oldPk, err := crypto.ParsePubKey(pubkey)
	if err != nil {
		return nil, err
	}

	var xorBytesScalar crypto.ModNScalar
	...
	var (
		newPointJacobian crypto.JacobianPoint
		oldPkJacobian    crypto.JacobianPoint
	)
...
func (net *netParams) ReadFrom(r io.Reader) (int64, error) {
	...
	switch wire.FlokicoinNet(binary.LittleEndian.Uint32(uint32Bytes)) {
	case wire.MainNet:
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	root, err := newRootFlcAddress(s, rootkey, nil, chaincode,
		createdAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) writeTo(w io.Writer) (n int64, err error) {
	...
	for _, wAddr := range s.addrMap {
		...
		case *scriptAddress:
			...
			copy(e.scriptHash160[:], flcAddr.AddrHash())
...
func (s *Store) Lock() (err error) {
	...
	for _, addr := range s.addrMap {
		if baddr, ok := addr.(*flcAddress); ok {
			_ = baddr.lock()
		}
	}
...
func (s *Store) ChangePassphrase(new []byte) error {
	...
	for _, wa := range s.addrMap {
		// Only flcAddresses curently have private keys.
		a, ok := wa.(*flcAddress)
		if !ok {
			continue
		}
...
func (s *Store) nextChainedAddress(bs *BlockStamp) (chainutil.Address, error) {
	addr, err := s.nextChainedFlcAddress(bs)
	if err != nil {
		return nil, err
	}
...
func (s *Store) nextChainedFlcAddress(bs *BlockStamp) (*flcAddress, error) {
...
func (s *Store) extendUnlocked(bs *BlockStamp) error {
	...
	newAddr, err := newFlcAddress(s, privkey, nil, bs, true)
	if err != nil {
		return err
	}
...
func (s *Store) extendLocked(bs *BlockStamp) error {
	...
	newaddr, err := newFlcAddressWithoutPrivkey(s, nextPubkey, nil, bs)
	if err != nil {
		return err
	}
	a = newaddr.Address()
...
func (s *Store) createMissingPrivateKeys() error {
	...
	prevAddr, ok := prevWAddr.(*flcAddress)
	if !ok {
		return errors.New("found non-pubkey chained address")
	}
...
func (s *Store) Address(a chainutil.Address) (WalletAddress, error) {
	...
	flcaddr, ok := s.addrMap[getAddressKey(a)]
	if !ok {
...
// Net returns the flokicoin network parameters for this key store.
...
func (s *Store) ImportPrivateKey(wif *chainutil.WIF, bs *BlockStamp) (chainutil.Address, error) {
	...
	flcaddr, err := newFlcAddress(s, privKey, nil, bs, wif.CompressPubKey)
	if err != nil {
		return nil, err
	}
	flcaddr.chainIndex = importedKeyChainIdx

	// Mark as unsynced if import height is below currently-synced
	// height.
	if len(s.recent.hashes) != 0 && bs.Height < s.recent.lastHeight {
		flcaddr.flags.unsynced = true
	}

	// Encrypt imported address with the derived AES key.
	if err = flcaddr.encrypt(s.secret); err != nil {
		return nil, err
	}

	addr := flcaddr.Address()
	...
	s.addrMap[getAddressKey(addr)] = flcaddr
	s.importedAddrs = append(s.importedAddrs, flcaddr)
...
func (s *Store) ExportWatchingWallet() (*Store, error) {
	...
	ws.keyGenerator = *(kgwc.(*flcAddress))
	...
	for apkh, addr := range s.addrMap {
		if !addr.Imported() {
			// Must be a flcAddress if !imported.
			flcAddr := addr.(*flcAddress)

			ws.chainIdxMap[flcAddr.chainIndex] =
				addr.Address()
		}
		apkhCopy := apkh
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// walletd shall refuse to open any unencrypted addresses.  This
...
// that walletd fits into an unused space left by Armory's key store file
// format.
...
// newFlcAddress initializes and returns a new address.  privkey must
// be 32 bytes.  iv must be 16 bytes, or nil (in which case it is
// randomly generated).
func newFlcAddress(wallet *Store, privkey, iv []byte, bs *BlockStamp, compressed bool) (addr *flcAddress, err error) {
	if len(privkey) != 32 {
		return nil, errors.New("private key is not 32 bytes")
	}

	addr, err = newFlcAddressWithoutPrivkey(wallet,
...
// newFlcAddressWithoutPrivkey initializes and returns a new address with an
...
// randomly generated).
func newFlcAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *flcAddress, err error) {
	var compressed bool
	switch n := len(pubkey); n {
	case secp.PubKeyBytesLenCompressed:
		compressed = true
...
// newRootFlcAddress generates a new address, also setting the
// chaincode and chain index to represent this address as a root
// address.
func newRootFlcAddress(s *Store, privKey, iv, chaincode []byte,
	bs *BlockStamp) (addr *flcAddress, err error) {

	if len(chaincode) != 32 {
		return nil, errors.New("chaincode is not 32 bytes")
	}

	// Create new flcAddress with provided inputs.  This will
	// always use a compressed pubkey.
	addr, err = newFlcAddress(s, privKey, iv, bs, true)
	if err != nil {
		return nil, err
	}
...
// unlocked flcAddress.
func (a *flcAddress) verifyKeypairs() error {
	if len(a.privKeyCT) != 32 {
		return errors.New("private key unavailable")
	}

	privKey, pubKey := crypto.PrivKeyFromBytes(a.privKeyCT)
...
func (a *flcAddress) ReadFrom(r io.Reader) (n int64, err error) {
	var read int64
...
func (a *flcAddress) WriteTo(w io.Writer) (n int64, err error) {
	var written int64
...
func (a *flcAddress) encrypt(key []byte) error {
...
func (a *flcAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	if !a.flags.encrypted {
	...
	_, pubKey := crypto.PrivKeyFromBytes(privkeyBytes)
...
func (a *flcAddress) changeEncryptionKey(oldkey, newkey []byte) error {
...
func (a *flcAddress) SyncStatus() SyncStatus {
	switch {
...
// PubKeyAddress.
func (a *flcAddress) PubKey() *crypto.PublicKey {
	return a.pubKey
}

func (a *flcAddress) pubKeyBytes() []byte {
	if a.Compressed() {
...
func (a *flcAddress) ExportPubKey() string {
...
func (a *flcAddress) PrivKey() (*crypto.PrivateKey, error) {
...
func (a *flcAddress) ExportPrivKey() (*chainutil.WIF, error) {
	...
	// elsewhere.
	wif, err := chainutil.NewWIF(pk, a.store.netParams(), a.Compressed())
	if err != nil {
		panic(err)
	}
	return wif, nil
...
// normal key store.
func (a *flcAddress) watchingCopy(s *Store) walletAddress {
	return &flcAddress{
		store:   s,
		address: a.address,
		flags: addrFlags{
...
func (a *flcAddress) setSyncStatus(s SyncStatus) {
	switch e := s.(type) {
	case Unsynced:
...
// address returns a chainutil.AddressScriptHash for a flcAddress.
