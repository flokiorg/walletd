...
func TestFlcAddressSerializer(t *testing.T) {
	...
	addr, err := newFlcAddress(fakeWallet, privKey, nil,
		makeBS(0), true)
	if err != nil {
		t.Error(err.Error())
		return
	...
	if !reflect.DeepEqual(addr, &readAddr) {
		t.Error("Original and read flcAddress differ.")
	}
}
...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Public keys (used to generate the flokicoin address) MUST match.
		...
		// the next pubkeys.
		data := "String to sign."
		privkeyUncompressed, _ := crypto.PrivKeyFromBytes(nextPrivUncompressed)
		privkeyCompressed, _ := crypto.PrivKeyFromBytes(nextPrivCompressed)
		sig := ecdsa.Sign(privkeyUncompressed, []byte(data))
		ok := sig.Verify([]byte(data), privkeyUncompressed.PubKey())
		if !ok {
			t.Errorf("%s: crypto signature verification failed for next keypair (chained from uncompressed pubkey).",
				test.name)
			return
		}
		...
		if !ok {
			t.Errorf("%s: crypto signature verification failed for next keypair (chained from compressed pubkey).",
				test.name)
			return
		}
	}
...
func TestWalletPubkeyChaining(t *testing.T) {
	...
	if !ok {
		t.Errorf("crypto signature verification failed; address's pubkey mismatches the privkey.")
		return
	}
	...
	ok = sig.Verify(hash, pubKey)
	if !ok {
		t.Errorf("crypto signature verification failed; next address's keypair does not match.")
		return
	}
...
func TestWatchingWalletExport(t *testing.T) {
	...
	pk, _ := crypto.PrivKeyFromBytes(make([]byte, 32))
...
func TestImportPrivateKey(t *testing.T) {
	...
	// import priv key
	wif, err := chainutil.NewWIF(pk, tstNetParams, false)
	if err != nil {
		t.Fatal(err)
	}
