...
walletd will not drop transaction history by itself, as this is something that
...
```
$ cd $GOPATH/src/github.com/flokiorg/walletd/cmd/dropwtxmgr
$ go get
```
...
```
$ dropwtxmgr
Database path: /home/username/.walletd/mainnet/wallet.db
Drop all walletd transaction history? [y/N] y
...
```
$ dropwtxmgr --db ~/.walletd/testnet/wallet.db
Database path: /home/username/.walletd/testnet/wallet.db
Drop all walletd transaction history? [y/N] y
Dropping wtxmgr namespace
```

After dropping transaction history, walletd may be restarted and a full rescan
will be triggered to sync the wallet:

```
$ walletd
14:05:31 2015-04-13 [INF] FLCW: No recorded transaction history -- needs full rescan
...
